function Restart-EcsRstNavService {
    [CmdletBinding()]
    param(
    ...
    try {
        ...
        # Restarting NAV Server Instance
        Write-EcsRstOutput "Restarting BC/NAV Server Instance"
        Write-EcsRstOutput ("========================================================")
        
        $serviceStartedSuccessfully = $false
        try {
            Restart-EcsDbcNavService -ServerInstance $bcServerInstance
        ...
        if ($serviceStartedSuccessfully) {
            Write-EcsRstOutput "BC/NAV Server Instance has been successfully restarted"
            Write-EcsRstOutput ("========================================================")
        }
        else {
            Write-EcsRstOutput "Failed to restart BC/NAV Server Instance. Check logs for more details."
            Write-EcsRstOutput ("========================================================")
        }
...
<## >
Restart-EcsRstNavService -restorePipelineID_arg 'AAA'
<##>