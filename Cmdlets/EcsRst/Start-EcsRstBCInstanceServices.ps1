function Start-EcsRstBCInstanceServices {
    [CmdletBinding()]
    param(
    ...
    try {
        ...
        # Starting BC/NAV Server Instance services
        Write-EcsRstOutput "Starting BC/NAV Server Instance services"
        Write-EcsRstOutput ("========================================================")
        
        # Check that services list is not empty
        if ( $null -eq ($global:Restore_ServicesToRestart).Count ) {
        Write-EcsRstOutput "There are no services to start for the environment. Skipping this step."
        Write-EcsRstOutput ("========================================================")    
        }
        else {
            try {
                Start-EcsDbcBCInstanceServices -EnvShortName $global:Restore_EnvShortName -Services $global:Restore_ServicesToRestart
            }
            catch {
                $errorMessage = $_
            ...
            Write-EcsRstOutput "BC/NAV Server Instance services have been successfully started"
            Write-EcsRstOutput ("========================================================")
        }
        # Show elapsed time
...
<## >
Start-EcsRstBCInstanceServices -restorePipelineID_arg 'AAA'
<##>