function Import-EcsRstSqlDatabaseRoles {
    [CmdletBinding()]
    param(
    ...
    try {
        ...
        try {
            ...
            if (-Not([string]::IsNullOrEmpty($global:Restore_SQLRoleMembershipFile))) {
                if (Test-Path -Path $global:Restore_SQLRoleMembershipFile -PathType Leaf) {
                    
                    Write-EcsRstOutput ("Importing database roles from file: $global:Restore_SQLRoleMembershipFile")
                    Import-EcsDbcSqlDatabaseRoles -inputFile_arg $global:Restore_SQLRoleMembershipFile -envShortName_arg $global:Restore_EnvShortName
                }
                else {
                    throw "ERROR: The current database roles backup file is not found: $global:Restore_SQLRoleMembershipFile"
                }
            }
            else {
                Write-EcsRstOutput ("The current database roles backup has not been taken by the pipeline. This step will be skipped")
            }
            
            Write-EcsRstOutput ("Database roles from file: '$global:Restore_SQLRoleMembershipFile' were imported successfully")
...
<## >
Import-EcsRstSqlDatabaseRoles -restorePipelineID_arg 'AAA'
<##>
