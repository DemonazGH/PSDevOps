function Start-EcsWinServiceAny {
    [CmdletBinding()]
    param (        
       ...
       ,[Parameter(HelpMessage = "Set 'Automatic' startup type before statring")]
        [switch]$setAutomatic
    ...
    try {
        ...
        # Enable service
        if ($setAutomatic) {
            Set-Service -Name $service_arg -StartupType Automatic -computername $computer_arg -ErrorAction Stop
        }
        ...
        {    
            ...
            switch($service.Status)
            {
                ...
                [System.ServiceProcess.ServiceControllerStatus]::Stopped) -contains $_ }
                {
                    ...
                    Write-Host "$msgPrefix_arg Starting service '$service_arg' on '$computer_arg'"
                    $service.Start()
                    break;
                }
        ...
        } while (($service.Status -ne [System.ServiceProcess.ServiceControllerStatus]::Running) -and ($waitCount -gt 0))        

        if ($service.Status -ne [System.ServiceProcess.ServiceControllerStatus]::Running) {
            throw "ERROR: Unable to start service '$service_arg' on server '$computer_arg'"
        }
        $serviceStatus = $service.Status
...
<## >
Start-EcsWinServiceAny -service_arg 'AOS60$01' -computer_arg FRSCBVAXHFX301D -setAutomatic
<##>